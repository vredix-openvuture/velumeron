...
// Bluetooth menu — grows out of the bar from the Bluetooth module (Flyout, like the player menu).
// Known (paired) devices list with connect/disconnect + a per-device gear (rename / forget); an
// "Add new" button switches to a scanned list of nearby devices to pair & connect. bluetoothctl-backed.
Flyout {
    id: root
    flyoutId: "bluetooth"
    panelW:   330
    ...
    property string mode:    "known"     // known | add
    ...
    onIsOpenChanged: if (isOpen) { mode = "known"; openMac = ""; refresh() }
