...
// Network menu — grows out of the bar from the Network module (Flyout, like the player / bluetooth
// menus). Wi-Fi radio toggle + scan/connect/disconnect/forget (inline password for secured, unknown
// networks) and a combined VPN section (nmcli vpn/wireguard connections, tap to toggle up/down).
Flyout {
    id: root
    flyoutId: "network"
    panelW:   330
    ...
    onIsOpenChanged: if (isOpen) { root.pwFor = ""; root.refresh() }
    function refresh() {
