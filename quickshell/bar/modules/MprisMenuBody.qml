...
// Mpris player content: square hi-res cover, track info, progress + transport controls. Hosted by
// MprisMenu (the standalone flyout) and by GroupMenu; `active` mirrors the host menu's open state
// and resets the pinned player each time the menu opens.
Column {
    id: root
    ...
    onActiveChanged: if (active) root.selected = null
    ...
    Column {
        ...
        Text {
            ...
            text:  root.player?.trackTitle ?? Wording.s("mpris.nothing")
    ...
    Column {
        ...
        Rectangle {
            width: parent.width; height: 6; radius: 3; color: Style.menuRowFill
            Rectangle {
