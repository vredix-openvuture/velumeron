...
| `audio-route.py` | Everything the sound dashboard needs that Quickshell doesn't expose: `move` / `move-source` a stream to another device (pactl's sink-input index is NOT the PipeWire node id — this owns the lookup), `default-sink` / `default-source`, `set-port`, `set-profile`, plus `streams` (which device each stream plays on + its media title) and `devices` (ports, card profiles, sample format, Bluetooth codec) |
...
| `$VELUMERON_USER_DIR/gui/local.json` | local-store.py ⇄ Disponera (shared local lists) |
