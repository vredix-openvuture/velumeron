...
# Default layout for side monitors (minimal: workspaces+submap left, clock right).
_default_side_bar_layouts=(
    "L:hyprland/workspaces,hyprland/submap|C:|R:clock"
)
...
# Minimal bar for side monitors: workspaces+submap left, clock right.
apply_side_bar() {
    local monitor="$1"
    local dest="$VUTURELAND_USER_DIR/waybar-modular/output/miboro/bar/top/$monitor"
    if [[ -f "$dest/style.css" ]] && grep -q "$VUTURELAND_USER_DIR" "$dest/style.css"; then
        local _cur _l _is_default=false
        _cur=$(_bar_layout "$dest/groups.json")
        for _l in "${_default_side_bar_layouts[@]}"; do
            [[ -n "$_cur" && "$_cur" == "$_l" ]] && { _is_default=true; break; }
        done
        if [[ "$_is_default" != true ]]; then
            ok "Side bar config for $monitor is customised — keeping it."
            return
        fi
    fi
    rm -rf "$dest"
    ...
    local module_dirs=(clock workspaces submap)
...
EOF
    ...
    ok "Created side bar for $monitor"
}
...
if [[ "$SYNC_MODE" == true ]]; then
    ...
    # Primary monitor gets the full bar; all others get the minimal side bar.
    if command -v hyprctl >/dev/null 2>&1 && hyprctl monitors -j >/dev/null 2>&1; then
        _sync_primary=$(grep -oP '^mon1\s*=\s*"\K[^"]+' "$USER_SETTINGS" 2>/dev/null | head -1 || true)
        [[ -z "$_sync_primary" ]] && _sync_primary=$(hyprctl monitors -j \
            | jq -r 'first(.[] | select(.focused==true) | .name) // .[0].name' 2>/dev/null || true)
        while IFS= read -r _mon; do
            [[ -z "$_mon" ]] && continue
            if [[ "$_mon" == "$_sync_primary" ]]; then
                apply_default_bar "$_mon"
            else
                apply_side_bar "$_mon"
            fi
